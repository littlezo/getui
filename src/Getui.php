 ...
 * @Since       : 2020-10-21 22:23:23
 * @LastAuthor  : @小小只 <soinco@qq.com>
 * @lastTime    : 2020-10-22 11:57:08
 ...
 * @FilePath    : \\pak\\getui\\src\\Getui.php
 */
/**
 * Created by PhpStorm.
 * User: Mr.Zhou
...
class Getui
{
     ...
     * @throws GeTuiException
     */
    public function pushSingleCid(array $params)
    {
        $ret = $this->requestCal('POST', $this->baseUri . '/push/single/cid', $params);
        if ($ret['result'] != 'ok') {
            return false;
        }
        return $ret;
     ...
     * @throws GeTuiException
     */
    public function pushSingleAlias(array $params)
    {
        $ret = $this->requestCal('POST', $this->baseUri . '/push/single/alias', $params);
        if ($ret['result'] != 'ok') {
            return false;
        }
        return $ret;
    ...
    {
        $ret = $this->requestCal('POST', $this->baseUri . '/push/single/batch', ['msg_list' => $params, 'need_detail' => $is_detail]);
        if ($ret['result'] != 'ok') {
            return false;
        }
        return $ret;
    ...
    public function bindAlias(array $params)
    {
        ...
        if ($ret['result'] != 'ok') {
            throw new GeTuiException($ret['desc']);
        }
        return true;
    }
    ...
    public function getAppUser($date)
    {
        ...
        if ($ret['result'] != 'ok') {
            throw new GeTuiException($ret['result']);
        }
        return $ret['data'];
    }
